...
-- Add batch_id column to photos table if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'photos' AND column_name = 'batch_id'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'photos' AND indexname = 'idx_photos_batch_id'
  ) THEN
